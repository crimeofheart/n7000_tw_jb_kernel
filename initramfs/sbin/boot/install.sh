...
#max cpu freq
if [ -f /data/wakeup/scaling_max_freq ]
then
 cat /data/wakeup/scaling_max_freq > /sys/devices/system/cpu/cpu0/cpufreq/scaling_max_freq
else
 cd /data
 toolbox mkdir wakeup
 toolbox chmod 777 /data/wakeup
 echo 1400000 > /data/wakeup/scaling_max_freq
 toolbox chmod 777 /data/wakeup/scaling_max_freq
 cat /data/wakeup/scaling_max_freq > /sys/devices/system/cpu/cpu0/cpufreq/scaling_max_freq
fi
echo "Complete! max cpu freq";

#min cpu freq
if [ -f /data/wakeup/scaling_min_freq ]
then
 cat /data/wakeup/scaling_min_freq > /sys/devices/system/cpu/cpu0/cpufreq/scaling_min_freq
else
 echo 200000 > /data/wakeup/scaling_min_freq
 toolbox chmod 777 /data/wakeup/scaling_min_freq
 cat /data/wakeup/scaling_min_freq > /sys/devices/system/cpu/cpu0/cpufreq/scaling_min_freq
fi
echo "Complete! min cpu freq";

#cpu governor
if [ -f /data/wakeup/scaling_governor ]
then
 cat /data/wakeup/scaling_governor > /sys/devices/system/cpu/cpu0/cpufreq/scaling_governor
else
 echo "HYPER" > /data/wakeup/scaling_governor
 toolbox chmod 777 /data/wakeup/scaling_governor
 cat /data/wakeup/scaling_governor > /sys/devices/system/cpu/cpu0/cpufreq/scaling_governor
fi
echo "Complete! change governor";

#I/O sheduler
if [ -f /data/wakeup/scheduler ]
then
 echo "user settings exist";
else
 echo "sfq" > /data/wakeup/scheduler
 toolbox chmod 777 /data/wakeup/scheduler
 echo "create default user settings";
fi
sched=`$b cat /data/wakeup/scheduler`
